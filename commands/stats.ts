...
function makePages(data: ISARdata): { [page_num: string]: Array<TField> } {
  ...
  return {
    "1": [{ name: "Solo", value: `Deaths: **${solo.deaths}**\nGames Played: **${solo.games_played}**\nKills: **${solo.kills}**\nMost Kills: **${solo.most_kills}**\nTop 5: **${solo.top_5}**\nWins: **${solo.wins}**\nKill / Death Ratio: \`${(solo.kills / (solo.deaths <= 0 ? 1 : solo.deaths)).toFixed(2)}\`\nWin / Loss Ratio: \`${((solo.wins / ((solo.games_played - solo.wins) <= 0 ? 1 : (solo.games_played - solo.wins))) * 100).toFixed(2)}\n\``, inline: false }, { name: "Duos", value: `Deaths: **${duos.deaths}**\nGames Played: **${duos.games_played}**\nKills: **${duos.kills}**\nMost Kills: **${duos.most_kills}**\nTop 3: **${duos.top_3}**\nWins: **${duos.wins}**\nKill / Death Ratio: \`${(duos.kills / (duos.deaths <= 0 ? 1 : duos.deaths)).toFixed(2)}\`\nWin / Loss Ratio: \`${((duos.wins / ((duos.games_played - duos.wins) <= 0 ? 1 : (duos.games_played - duos.wins))) * 100).toFixed(2)}\``, inline: false }, { name: "Squads", value: `Deaths: **${squads.deaths}**\nGames Played: **${squads.games_played}**\nKills: **${squads.kills}**\nMost Kills: **${squads.most_kills}**\nTop 2: **${squads.top_2}**\nWins: **${squads.wins}**\nKill / Death Ratio: \`${(squads.kills / (squads.deaths <= 0 ? 1 : squads.deaths)).toFixed(2)}\`\nWin / Loss Ratio: \`${((squads.wins / ((squads.games_played - squads.wins) <= 0 ? 1 : (squads.games_played - squads.wins))) * 100).toFixed(2)}\``, inline: false }],
